...
class Publish extends Component{
    ...
    state = {
        ...
        async  componentDidMount() {
            const response = await fetch('http://localhost:8080/posts/admin/publish',
            {
                method:"Get",
                headers:{
                        ...
                        <div class="lg:grid grid-cols-3 space-x-5 space-y-5">
                                    {/* {posts && posts.length > 0 && posts.map(({post}) => */}
                                        ...
                                        <div class="bg-gray-100 border border-black md:border-none block p-4 text-sm" key={post.post_id}>
                                            ...
                                            <div class="">
                                                <a href={`/Dashboard/PublishPost/${post.post_id}`} key={post.post_id} class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 border border-red-500 rounded w-24">See post</a>
