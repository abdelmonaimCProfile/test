...
function AllAdmins() {
    ...
    const USER_API_BASE_URL="http://localhost:8080/users/admins";
    const [ignored,forceUpdate]=useReducer(x=>x+1,0);
    const [admins ,setAuthors]=useState(null);
                ...
                <LayoutAuthenticated>
                    <div class="overflow-x-scroll">
                        <div class="overflow-x-scroll">
                            ...
                            <caption class="text-6xl text-bold m-4 text-left ">Admins</caption>
                            ...
                            {!Loading && (
                                <tbody class="block md:table-row-group">
                                    {/* {admins && admins.length > 0 && admins.map(({author}) => */}
                                    {admins.map((admin) =>
                                        <tr class="bg-gray-100 border border-black md:border-none block md:table-row">
                                            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">{admin.user_first_name}</td>
                                            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">{admin.user_last_name}</td>
                                            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">{admin.user_email_adresse}</td>
                                            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">{admin.user_password}</td>
                                            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">{admin.user_creation_date}</td>
                                            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">{admin.user_birthday}</td>
                                            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">{admin.user_city}</td>
                                            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">{admin.user_country}</td>
                                            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">{admin.user_phone_number}</td>
                                            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">{admin.user_twitter}</td>
                                            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">{admin.user_facebook}</td>
                                            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">{admin.user_instagram}</td>
                                            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">{admin.user_linkedin}</td>
                                            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">
                                                <span class="inline-block w-1/3 md:hidden font-bold">Actions</span>
                                                <div class="flex">
                                                <a href={`/Dashboard/AllAdmins/${admin.user_id}`}   key={admin.user_id}  class="">
                                                ...
                                                <button onClick={() =>remove(admin.user_id)} class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 border border-red-500 rounded w-10">
